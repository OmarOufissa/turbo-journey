...
import type { Division, Service, Equipe, Agent, ArticleReference } from "@shared/api";
...
export function AffecterDialog({ open, onClose, initial, onSuccess }: { open: boolean; onClose: () => void; initial?: AffecterInitial; onSuccess?: () => void }) {
  ...
  const [beneficiaireType, setBeneficiaireType] = useState<"agent" | "equipe">(initial?.beneficiaire?.type ?? "agent");
  ...
  const canRecap = articleId != null && (beneficiaireType === "agent" ? agentId != null : equipeId != null) && dateAffectation;
      ...
      <DialogContent className="max-w-xl">
        ...
        {step === "form" && (
          <div className="max-h-[65vh] space-y-5 overflow-y-auto pr-1">
            ...
            {articleInfo && (
              <div className="rounded-md border border-dashed p-3 text-sm">
                ...
                <div className="mt-1 grid grid-cols-2 gap-x-4 gap-y-0.5 text-xs text-muted-foreground">
                  <span>N° de série : {articleInfo.numeroSerie ?? "—"}</span>
            ...
            {!beneficiairePrefilled && (
              <section className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button type="button" size="sm" variant={beneficiaireType === "agent" ? "default" : "outline"} onClick={() => setBeneficiaireType("agent")}>Agent (EPI)</Button>
                  <Button type="button" size="sm" variant={beneficiaireType === "equipe" ? "default" : "outline"} onClick={() => setBeneficiaireType("equipe")}>Équipe (EPC)</Button>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Select value={divisionId != null ? String(divisionId) : "all"} onValueChange={(v) => { setDivisionId(v === "all" ? null : Number(v)); setServiceId(null); setEquipeId(null); setAgentId(null); }}>
                  ...
                  <Select value={serviceId != null ? String(serviceId) : "all"} onValueChange={(v) => { setServiceId(v === "all" ? null : Number(v)); setEquipeId(null); setAgentId(null); }}>
                  ...
                  <Select value={equipeId != null ? String(equipeId) : "all"} onValueChange={(v) => { setEquipeId(v === "all" ? null : Number(v)); setAgentId(null); }}>
            ...
            {beneficiairePrefilled && (
              <div className="rounded-md border border-dashed p-3 text-sm">
                Bénéficiaire : <span className="font-medium">{beneficiaireType === "agent" ? agentDetail?.nom : equipeDetail?.nom}</span>
              </div>
            )}

            <section className="space-y-3">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">3. Informations d'affectation</p>
        ...
        {step === "recap" && (
          <div className="space-y-3 text-sm">
            ...
            <div className="rounded-md border p-3">
              ...
              <p><span className="text-muted-foreground">Bénéficiaire :</span> {beneficiaireType === "agent" ? agentDetail?.nom : equipeDetail?.nom} ({beneficiaireType === "agent" ? "Agent — EPI" : "Équipe — EPC"})</p>
